...
// raises IRQ1 when it has data available for reading.
#define REG_KB_DATA 0x60
// Command port for keyboard controller, used to enable/disable kbd interrupts,
// etc.
#define REG_KB_CMD 0x64
...
void kb_handler(struct regs *r) {
  unsigned char scancode = port_byte_in(REG_KB_DATA);

  if (scancode & 0x80) {
    // Key has been released if top bit of byte is set
  } else {
    // Key has been pressed
    print_char(kb_us_keymap[scancode], 0);
  }
}

// Install the custom kb handler in our IRQ based interrupt mapping.
void kb_install() {
  ...
  // All the rest of the keys are undefined
