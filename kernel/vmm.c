...
#define PROCESS_HEAP_START 0x80000000
#define PROCESS_HEAP_END 0x90000000
...
static uint32_t is_pde_empty(uintptr_t va) {
  ...
  return ((uintptr_t)(((Pd*)((uintptr_t)PD_RECURSIVE_I << VA_PDI_START | PD_RECURSIVE_I << VA_PTI_START))->pts[pde_i]) & PT_PRESENT) == 0;
}
...
static uint32_t is_pte_empty(uintptr_t va) {
  ...
  return ((uintptr_t)(((Pd*)((uintptr_t)PD_RECURSIVE_I << VA_PDI_START | pde_i << VA_PTI_START))->pts[pte_i]) & PT_PRESENT) == 0;
}
...
static void create_pde(uintptr_t va) {
  ...
  uint32_t pde_i = va_to_pde_i(va);
  (((Pd*)((uintptr_t)PD_RECURSIVE_I << VA_PDI_START | PD_RECURSIVE_I << VA_PTI_START)))->pts[pde_i] = (Pt*)((uintptr_t)alloc_frame() | PT_PRESENT | PT_WRITE);
}
...
static void create_pte(uintptr_t va, uintptr_t frame) {
  ...
  (((Pt*)((uintptr_t)PD_RECURSIVE_I << VA_PDI_START | pde_i << VA_PTI_START)))->frames[pte_i] = (uintptr_t)(frame | PT_PRESENT | PT_WRITE);
}
...
ProcessPd* create_process_pd() {
  ...
  process_pd->pd_va = curr == process_pds ? (Pd*)K_PAGE_START : curr->pd_va + 1;
  ...
  /* Recursive Entry */
  process_pd->pd_va->pts[PD_RECURSIVE_I] = (Pt *)(process_pd->pd_pa | PT_USER | PT_WRITE | PT_PRESENT);
  ...
  for (int i = kernel_pde; i < 1023; i++) {
    if (process_pd->pd_va->pts[i] != process_pds->pd_va->pts[i]) {
      process_pd->pd_va->pts[i] = (Pt*)(((uintptr_t)process_pds->pd_va->pts[i] & NO_FLAG_MASK) | PT_PRESENT);
    }
  }
...
- The higher-half kernel code is linked at VA 0xC0000500 and is mapped to PA 0x500.
...
void vm_init() {
  ...
  (((Pd*)(PD_RECURSIVE_I << VA_PDI_START | PD_RECURSIVE_I << VA_PTI_START)))->pts[0] = 0x0;
  flush_tlb();
...
// vm_range_t* create_process_heap(){
//   return vmm_init(PROCESS_HEAP_START);
// }
...
int kfree(void* va) {
  ...
  if (va_node < (VmNode*) K_HEAP_START || (uintptr_t)va_node + sizeof(VmNode) > K_HEAP_END || 
      (uintptr_t)va_node + sizeof(VmNode) + va_node->no_bytes > K_HEAP_END) {
    return 1;
  }

  /* Free node */
  mem_set((uint8_t*)((uintptr_t)va_node) + sizeof(VmNode), 0x0, va_node->no_bytes);
  va_node->flags = 0x0;
...
VmRange* vmm_init(uintptr_t vm_range_start){
  ...
  vm_range->head = (VmNode*)(vm_range_start + sizeof(VmRange*) + sizeof(VmRange));

  /* Head */

  uint32_t available_bytes = PAGE_SIZE - sizeof(VmRange*) - sizeof(VmRange) - sizeof(VmNode);
  ...
  new_vm_range = (VmNode*)((uintptr_t)(vm_range->head) + sizeof(VmNode) + vm_range->head->no_bytes);
...
uint8_t vmm_alloc(VmRange* vm_range, uint8_t flags, uintptr_t arg){
  ...
  VmNode* new_vm_range = (VmNode*)((uintptr_t)(vm_range->tail) + sizeof(VmNode) + vm_range->tail->no_bytes);
  ...
  if (vm_range->head->next != vm_range->tail && (vm_range->tail->prev->flags & VM_USED) == 0) {
    ...
    left->next = NULL;
    mem_set((uint8_t*)vm_range->tail, 0x0, sizeof(VmNode) + vm_range->tail->no_bytes);
    vm_range->tail = left;
  }
